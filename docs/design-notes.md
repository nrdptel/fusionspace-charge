...
The Open Graph / Twitter card is a committed static PNG at `public/og.png`, regenerated by
`scripts/gen-og.mjs` (run locally, not in CI — matching how the Fusion Space landing site ships
its card). It follows the **landing card's composition** rather than the Motor Finder's denser
one: the brand mark over the product name, a one-line tagline, and the domain in mono, centered
on the `#09090b` background with the indigo radial glow — quieter and more professional than a
left-aligned eyebrow/subtitle block. It's rendered with Chromium (via Playwright) because that
renders the gradient mark and the radial glow faithfully, where `next/og`/Satori does not.
